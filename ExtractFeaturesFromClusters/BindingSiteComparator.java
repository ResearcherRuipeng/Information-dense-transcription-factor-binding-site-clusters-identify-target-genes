...
public class BindingSiteComparator implements Comparator<BindingSite> {
	public int compare(BindingSite site1, BindingSite site2) {
		if(site1.getPosition() < site2.getPosition()) {
			return -1;
		}
		else if(site1.getPosition() > site2.getPosition()) {
			return 1;
		}
		else {
			if(site1.getSequence().length() < site2.getSequence().length())
			{
				return -1;
			}
			else if(site1.getSequence().length() > site2.getSequence().length())
			{
				return 1;
			}
			else
