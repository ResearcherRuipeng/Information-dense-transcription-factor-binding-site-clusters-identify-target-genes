...
public class GeneratePromoterIntervalFile
{
	...
	{
		String allGeneFilePath = args[0];
		String inputFilePath = args[1];
		//String ifPromoterRegionsCanBeProducedString = args[2];
		...
		TreeMap<String, String> aSubsetOfGenes = readASubsetOfGenes(inputFilePath);
		
		//boolean ifPromoterRegionsCanBeProduced = Boolean.parseBoolean(ifPromoterRegionsCanBeProducedString);
		...
		try
		{
			...
			{		
				...
				String coeffOfThisGene = anEntry.getValue();
				//System.out.println(emsembleNoOfAGene);
				...
				else
				{
					for(int i = 0; i < allTranscriptsOfAGene.size(); i++)
					{
						...
						/*if(!anEntry.getValue().equals(fields[4]))
					{
						System.out.println("The gene names of the Ensembl No. " + emsembleNoOfAGene + " don't match in the file containing all genes and TableS3.txt.");
						System.exit(1);
						break;
					}*/
						//if(ifPromoterRegionsCanBeProduced)
						//{
					...
					{
						String currentGene = "chr" + chromosome + "\t" + (smallCoor.get(m) - 1) + "\t" + largeCoor.get(m) + "\t" + geneName + "\t" + geneID + "\t" + coeffOfThisGene + "\t" + geneType + "\t" + strand + "\n";
						output.print(currentGene);				
					}
					System.out.println("Number of genes that were processed: " + (++numberOfGenes));
				}
			}	
		...
		{
			System.out.println("Uable to open output file.");
			System.exit(1);
		}
	...
	private static TreeMap<String, String> readASubsetOfGenes(String geneListFilePath)
	{
		...
		try
		{
			...
			while(inputer.hasNextLine())
			{
				...
				String[] fields = aLine.split(",");
				aSubsetOfGenes.put(fields[0], fields[2]);
			}
			inputer.close();
		}
		catch(Exception e)
	...
	private static void processOneGene()
	{
		if(strand.equals("1"))
		{
			boolean done = false;						
